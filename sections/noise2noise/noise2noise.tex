...
\subsection{Toward the Natural Exponential Family}
...
This result can be extended to any noise model belonging to the exponential family of distributions.
...
\begin{corollary}
    If $\Delta_\rho(z, x)$ is independent of $\theta$, then $\text{argmin}_{\theta} R^{\text{N2N}}(\theta) = \text{argmin}_\theta R^{\text{clean}}(\theta)$.
\end{corollary}

As we already saw in the case of the MSE loss under gaussian noise and the Poisson negative log-likelihood loss under Poisson noise, this additional term is respectively equal to the variance of the noise and a constant term independent of $\theta$.
We can show that this property holds for any noise model belonging to the exponential family of distributions and for the corresponding negative log-likelihood loss function.

\begin{definition}
    A distribution belongs to the exponential family if its probability density function can be written as :
    \begin{equation*}
        p(y|x) = h(y) \exp(\eta(x)^T T(y) - A(\eta(x)))
    \end{equation*}
    where $T(y)$ is the sufficient statistic, $\eta(x)$ is the natural parameter, $A(\eta(x))$ is the log-partition function and $h(y)$ is the base measure.
\end{definition}

\subsection{Particular case of the MAE loss}
