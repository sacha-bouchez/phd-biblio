...
More rigorously, Definition \ref{def:n2n_compatible} formalizes the notion of Noise2Noise-compatible loss function and corruption process, which is a necessary condition for the Noise2Noise framework to be equivalent to the supervised learning framework.
\begin{definition}{Noise2Noise property}
    \label{def:n2n_compatible}
    Let $(\rho, \mathcal{C})$ be a couple of loss function and corruption process.
    The Noise2Noise framework is equivalent to its supervised counterpart under some conditions on $(\rho, \mathcal{C})$ which satisfies the \ref{eq:n2n_compatible} Property if
    the minimization of \eqref{eq:n2n_loss} gives the same estimator as the minimization of \eqref{eq:supervised_loss} as $N \to \infty$ :
    \begin{equation}
        \label{eq:n2n_compatible}
        \tag{N2N}
        \argmin_\theta \mathbb{E}_{Y^{(1)}, Y^{(2)}}[\rho(f_\theta(Y^{(1)}), Y^{(2)})] = \argmin_\theta \mathbb{E}_{Y, X}[\rho(f_\theta(Y), X)]
    \end{equation}
\end{definition}
\begin{figure}
    \centering
...
\subsection{Theoretical guarantees of Noise2Noise}

\paragraph{MSE loss case}

The Minimum Squared Error (MSE) loss function is a very common choice for denoising tasks. It is defined as follows :
\begin{equation}
    \rho(x, y) = \| x - y \|_2^2    
\end{equation}
\noindent We show here that the MSE loss function has interesting properties regarding Noise2Noise.
...
\begin{proposition}
    ...
    If $Y^{(1)}$ and $Y^{(2)}$ are independent given the clean image $X$ (i.e. $Y^{(1)} | X \independent Y^{(2)} | X$),
    the MSE associated with any centered noise distribution, satisfy the Noise2Noise property \eqref{eq:n2n_compatible}.
\end{proposition}
\begin{proof}
    We expand the Noise2Noise empirical risk as follows :
    \begin{align*}
        R_{N, 1}^{N2N}(\theta) &= \frac{1}{N} \sum_{i=1}^N \left[ \| f_\theta(Y_i^{(1)}) - Y_i^{(2)} \|_2^2 \right] \\
        &= \frac{1}{N} \sum_{i=1}^N \left[ \| f_\theta(Y_i^{(1)}) - X + (X - Y_i^{(2)}) \|_2^2 \right] \\
        &= R_{N, 1}^{clean}(\theta) + \frac{1}{N} \sum_{i=1}^N \left[ \| X - Y_i^{(2)} \|_2^2 \right] + \frac{2}{N} \sum_{i=1}^N \left[ (f_\theta(Y_i^{(1)}) - X)^T (X - Y_i^{(2)}) \right]
    \end{align*}
    However, $\frac{2}{N} \sum_{i=1}^N \left[ (f_\theta(Y_i^{(1)}) - X)^T (X - Y_i^{(2)}) \right] \xrightarrow[N \to \infty]{a.s.} 2 \mathbb{E} \left[ (f_\theta(Y^{(1)}) - X)^T (X - Y^{(2)}) \right]$ by the law of large numbers. This expectation vanishes under the independence assumptions :
    \begin{equation*}
        R_{N, 1}^{N2N}(\theta) = R_{N, 1}^{clean}(\theta) + \mathbb{E} \left[ \| X - Y^{(2)} \|_2^2 \right] + o(1)
    \end{equation*}
    the second term being an irreductible variance term independant of $\theta$.
\end{proof}
...
\begin{proposition}
    ...
    If $Y^{(1)}$ and $Y^{(2)}$ are independent given the clean image $X$ (i.e. $Y^{(1)} | X \independent Y^{(2)} | X$), then $\rho = \mathcal{L}_{Poisson}$ associated with Poisson noise model ($Y | X=x \sim \text{Poisson}(x)$) satisfy the Noise2Noise property \eqref{eq:n2n_compatible}.
\end{proposition}
...
\begin{corollary}
    \label{coro:delta}
    The Noise2Noise property \eqref{eq:n2n_compatible} is satisfied if and only if $\Delta_\rho(z, x)$ is independent of $z$.
\end{corollary}

We can give a new demonstration of Proposition \ref{prop:noise2noise_l2} using the noise-induced loss bias formalism with the MSE loss function :
\begin{align*}
...
\end{align*}
\noindent According to corollary \ref{coro:delta}, we need this quantity to be independent of $z$. Thus, we need the term $\langle z - x, \mathbb{E}[x - Y | X=x] \rangle$ to vanish for all $z$, which is equivalent to having $\mathbb{E}[Y | X=x] = x$.

In summary, we have proven that the Noise2Noise framework, combined with the use of the MSE loss function, is valid for any noise model as long as the noise is centered around its clean counterpart,
in addition to the independence assumption between the two noisy observations given the clean image.
For example, if $Y|X=x$ follows a log-normal distribution, then $\mathbb{E}[Y|X=x] = x \exp(\sigma^2/2) \neq x$ and the Noise2Noise property \eqref{eq:n2n_compatible} doesn't hold in this case.
...
\subsection{Toward the exponential family of distributions}
...
\begin{proposition}
    \label{prop:ef_n2n}
    A noise model belonging to the exponential family associated with its corresponding negative log-likelihood loss function satisfies the Noise2Noise property \eqref{eq:n2n_compatible} if and only if the sufficient statistic $T(Y)$ is an unbiased estimator of $T(X)$ :
    \begin{equation}
        \mathbb{E}[T(Y) | X=x] = T(x)
    \end{equation}
\end{proposition}
\noindent Note that this property of the sufficient statistics is satisfied for Gaussian and Poisson distributions as we have already seen in Proposition \ref{prop:noise2noise_l2} and \ref{prop:noise2noise_poisson}
...
\begin{proposition}
    ...
    If noise is centered, i.e. $\mathbb{E}[Y | X=x] = x$, then \textbf{target-first} Bregman divergence associated to any noise model satisfies the Noise2Noise property \eqref{eq:n2n_compatible}.
\end{proposition}
\begin{proof}
...
\begin{proposition}
    A \textbf{prediction-first} Bregman divergence associated with any noise model satisfies the Noise2Noise property \eqref{eq:n2n_compatible} if and only if the following condition holds :
    \begin{equation}
...
This is basically a generalization of Proposition \ref{prop:noise2noise_l2}.
